...
class bilibiliClient():
    ...
    async def SendJoinChannel(self, channelId):
        self._uid = (int)(1e14 + 2e14 * random.random())
    ...
    def parseDanMu(self, messages):
        ...
        if cmd == 'DANMU_MSG':
            ...
            try:
                print (time.strftime("%H:%M:%S",time.localtime()) +' '+ commentUser + ' say: ' + commentText)
            except:
                pass
            return
        if cmd == 'SEND_GIFT' and config.TURN_GIFT == 1:
            ...
            try:
                print(time.strftime("%H:%M:%S",time.localtime()) +' '+ GiftUser + ' 送出了 ' + str(GiftNum)+ '个' + GiftName)
            except:
                pass
            return
        if cmd == 'WELCOME' and config.TURN_WELCOME == 1:
            ...
            try:
                print (time.strftime("%H:%M:%S",time.localtime()) +' '+ '欢迎 ' + commentUser + ' 进入房间')
            except:
                pass
            return
        return
